...
def yield_cases(files):
    ...
    for x in files:
        ...
        case["docs"] = [
            x.text for x in doc.any_xpath(".//tei:list[1]/tei:item/tei:ref")
        ]
        case["description"] = doc.any_xpath('.//tei:abstract//text()')
        ...
        case["end_date"] = date_from_tei(
            f"./data/editions/{case['last_doc']}", default=""
        )
        for y in case["docs"]:
            cur_doc = None
            item = {}
            ...
            try:
                item["facs"] = cur_doc.any_xpath(
                    './/tei:graphic[@source="wienbibliothek"]/@url'
                )[0]
            except IndexError:
                item["facs"] = "no facs"
